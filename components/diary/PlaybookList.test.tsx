...
describe("PlaybookList", () => {
  it("기록이 없으면 빈 상태 메시지를 표시한다", () => {
    render(<PlaybookList playbook={null} hasEntries={false} />);
    expect(screen.getByText("아직 기록이 없어요.")).toBeInTheDocument();
  });

  it("기록이 있고 playbook 항목이 있으면 번호 매긴 목록을 표시한다", () => {
    render(<PlaybookList playbook={playbook} hasEntries={true} />);
  ...
  it("기록은 있지만 playbook 항목이 비어있으면 빈 상태를 표시한다", () => {
    ...
    render(<PlaybookList playbook={empty} hasEntries={true} />);
    expect(screen.getByText("아직 기록이 없어요.")).toBeInTheDocument();
  });
});
